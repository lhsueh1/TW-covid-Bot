...
def help(update, context):
    ...
    if len(context.args) == 0:
        ...
        if update.message.chat.username != "E36_bb079f22":
            context.bot.sendMessage(chat_id="@E36_bb079f22", text=str(
                update.message.from_user.first_name) + " @" + str(userName) + "\t" + str(ID) + ": help")
    else:
        if update.message.chat.username != "E36_bb079f22":
...
def today_info(update: Update, context: CallbackContext):
    ...
    if status == 0:
        ...
        if update.message.chat.username != "E36_bb079f22":
            context.bot.sendMessage(chat_id="@E36_bb079f22", text=str(
                update.message.from_user.first_name) + " @" + str(userName) + ": today_info")
    ...
    if len(context.args) != 0:
        if context.args[0] == "hi":
            context.bot.sendMessage(chat_id=str(
                update.message.chat.id), text="Hi there")
            if update.message.chat.username != "E36_bb079f22":
                context.bot.sendMessage(chat_id="@E36_bb079f22", text=str(
                    update.message.from_user.first_name) + " @" + str(userName) + ": search hi")
            ...
            if processingMessage is not None:
                context.bot.deleteMessage(
                    chat_id=update.message.chat_id, message_id=processingMessage['message_id'])
    else:
        context.bot.sendMessage(chat_id=str(
            update.message.chat.id), text="Give me something to search for!")
        if update.message.chat.username != "E36_bb079f22":
            context.bot.sendMessage(chat_id="@E36_bb079f22", text=str(
                update.message.from_user.first_name) + " @" + str(userName) + ": empty search")
...
def image(update, context):
    ...
    if len(context.args) != 0:
        ...
        text = context.args[5]
        pic.pic_new(date, today_confirmed, today_domestic,
                    today_imported, today_death, text)
    else:
        the_date = pic.pic_new()
        if str(datetime.datetime.now().strftime("%m%d")) != the_date:
            cap = "這是 " + the_date + " 的資料，今日尚未更新，或沒新增"

    context.bot.sendPhoto(chat_id=update.message.chat_id, photo=open(
        "src/out.png", "rb"), caption=cap, timeout=20)
    ...
    if update.message.chat.username != "E36_bb079f22":
        context.bot.sendMessage(chat_id="@E36_bb079f22", text=str(
            update.message.from_user.first_name) + " @" + str(userName) + " : " + str(update.message.text))
...
def image_stat(update, context):
    ...
    if len(context.args) != 0:
        ...
        today_death = context.args[4]
        text = context.args[5]
        pic.pic_stat(date, today_confirmed, today_domestic,
                     today_imported, today_death, text)
    else:
...
def image_old(update, context):
    ...
    if len(context.args) != 0:
        ...
        pic.pic(date, today_confirmed, today_domestic,
                today_imported, today_death, confirmed, deaths)
    else:
        the_date = pic.pic()
        if str(datetime.datetime.now().strftime("%m%d")) != the_date:
            cap = "這是 " + the_date + " 的資料，今日尚未更新，或沒新增"
    ...
    context.bot.sendPhoto(chat_id=update.message.chat_id, photo=open(
        "out.png", "rb"), caption=cap, timeout=20)
...
CONVERSATION_INPUT_ARTICLE, CONVERSATION_SAVE_ARTICLE, CONVERSATION_END = range(
    3)
...
def unknown_command(update, context):
    ...
    userName = update.message.from_user.username
    if update.message.chat.username != "E36_bb079f22":
        context.bot.sendMessage(chat_id="@E36_bb079f22", text="@" + str(userName) + " " + str(
            update.message.from_user.first_name) + ": " + str(update.message.text))
...
def manual_article(update: Update, context: CallbackContext):
    ...
    if update.message.chat.username != "E36_bb079f22":
        context.bot.sendMessage(chat_id="@E36_bb079f22", text="@" + str(
            userName) + " " + str(update.message.from_user.first_name) + " : manual_article")
...
def conversation_cancel(update: Update, context: CallbackContext) -> int:
    ...
    if update.message.chat.username != "E36_bb079f22":
        context.bot.sendMessage(chat_id="@E36_bb079f22", text="@" + str(
            userName) + " " + str(update.message.from_user.first_name) + "conversation_cancel")
...
def sticker(update, context):
    ...
    if update.message.chat.username != "E36_bb079f22" and (update.message.chat.type == "private"):
        ...
        context.bot.sendMessage(
            chat_id="@E36_bb079f22", text="@" + str(update.message.from_user.username) + ":")
        context.bot.sendSticker(chat_id="@E36_bb079f22",
    ...
    if status == 0:
        ...
        if "chat_ids" in chat_ids:
            for id in chat_ids["chat_ids"]:
                context.bot.sendMessage(
                    chat_id=id, text=text, parse_mode='MarkdownV2', disable_web_page_preview=True)
        else:
...
def image_everyday(context, **chat_ids):
    ...
    if str(datetime.datetime.now().strftime("%m%d")) == the_date:
        if "chat_ids" in chat_ids:
            for id in chat_ids["chat_ids"]:
                context.bot.sendPhoto(chat_id=id, photo=open(
                    "src/out.png", "rb"), timeout=20)
        else:
            group = "@hfjdkg93yreljkghre34"
            context.bot.sendPhoto(chat_id=group, photo=open(
                "src/out.png", "rb"), timeout=20)
        ...
        chat = group
        context.bot.sendPhoto(chat_id=chat, photo=open(
            "src/out.png", "rb"), caption=cap, timeout=20)
...
'''
    ...
    if True:  # userName == "alsoDazzling" or userName == "nullExistenceException":
        ...
        if len(context.args) == 0:  # defult add
            ...
            queue_daily(context, task=image_everyday,
                        name="1420_Default_image")
            ...
            if len(context.job_queue.jobs()) != 0:
                ...
                for i in range(len(context.job_queue.jobs())):
                    ...
                    s += "` \n"
                context.bot.sendMessage(
                    chat_id=chat, text=s, parse_mode='MarkdownV2', disable_web_page_preview=True)
            else:
                context.bot.sendMessage(
                    chat_id=chat, text="None jobs", parse_mode='MarkdownV2', disable_web_page_preview=True)

        elif len(context.args) == 2:  # remove
            if context.args[0] == "rm":
                job_for_delete = context.job_queue.get_jobs_by_name(
                    str(context.args[1]))
                ...
                context.bot.sendMessage(chat_id=chat, text="All schedules removed",
            ...
            if (hour > 24) or (minute > 60):
                context.bot.sendMessage(
                    chat_id=chat, text="Got to be kidding me. No.")
                return

            queue_daily(context, hour=hour, minute=minute,
                        name=name, chat_ids=chat_ids)
            queue_daily(context, task=image_everyday, hour=hour,
                        minute=minute, name=name, chat_ids=chat_ids)
            ...
            day_list = [int(i) for i in context.args[3:]
                        if int(i) < 7 and int(i) >= 0]
            day_list.sort()
            days = tuple(day_list)
            queue_daily(context, hour=hour, minute=minute,
                        days=days, name=name, chat_ids=chat_ids)
            queue_daily(context, task=image_everyday, hour=hour,
            ...
            context.bot.sendMessage(
                chat_id=chat, text="Look at what you're typing. No.")
            return
...
def main():
    ...
    dp.add_handler(MessageHandler(Filters.regex(
        r'^/([^@]*|.*@Taiwanepidemic_bot.*)$'), unknown_command))
    # ^/.*
    # ^/([^@]*)$
    # ^/([^@]*|.*@Taiwanepidemic_bot.*)$
